...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        if isinstance(m, (Detect, IDetect, Segment, ISegment, IRSegment)):
            ...
            m.stride = torch.tensor([s / x.shape[-2] for x in forward(torch.zeros(1, ch, s, s))])  # forward  此处修改torch.zeros(1, ch, s, s)
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in {
                ...
                ODConv2d, DyHeadBlock, DCNv2, SAConv2d, CoordConv, DSConv2D, PConv, CARAFE, BiLevelRoutingAttention, Attention, AttentionLePE,
                Conv_ATT, Conv_Swin, Conv_Cross, Conv_Tran, Conv_Conv, BiLevelRoutingAttention, Attention, AttentionLePE,
                SwinTransformerBlock, STCSPA, STCSPB, STCSPC, Conv2Formers,
                SwinTransformer2Block, ST2CSPA, ST2CSPB, ST2CSPC, TransformerBlock, CrossFormer, CoT3}:
            c1, c2 = ch[f], args[0]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='/root/seg/models/segment/test2.yaml', help='model.yaml')
